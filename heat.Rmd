...
# rm(me_rural_sf, me_urban_sf)
# me_blkgrpLST_sf <- me_blkgrpLST_sf %>% 
#   select(!(OBJECTID_15:Shape_Area))

me_blkgrpLST_sf <- readRDS(file = "../DATA/me_blkgrps_sf_CUM.Rds")

#create layer of municipalities
me_towns <- county_subdivisions("ME") %>% 
  filter(NAME != "County subdivisions not defined") %>%  
  st_transform(., crs = 4326) %>% 
  select(NAME,NAMELSAD)

# Assign municipality names to block groups
me_blkgrpLST_sf <- county_subdivisions("ME") %>% 
  st_transform(., crs = st_crs(me_blkgrpLST_sf)) %>% 
  transmute(TOWN = NAMELSAD) %>% 
  st_join(me_blkgrpLST_sf, ., largest = TRUE) %>% 
  mutate(NAME = str_remove_all(NAME, ", Maine")) %>% 
  st_transform(., crs = 4326)

# create layer of state house districts
house_districts <- st_read("../DATA/shapefiles/Maine_House_Districts_2013_GeoLibrary-shp", "Maine_House_Districts_2013_GeoLibrary") %>%
  transmute(REP_DIST = HD2013) %>% 
  st_transform(., crs = 4326) %>%
  st_make_valid()

# create layer of state senate districts
senate_districts <- st_read("../DATA/shapefiles/Maine_State_Senate_Districts_2013_GeoLibrary-shp", "Maine_State_Senate_Districts_2013_GeoLibrary") %>%
  transmute(SEN_DIST = SD2013) %>% 
  st_transform(., crs = 4326) %>%
  st_make_valid()

# Assign state house district names to block groups
me_blkgrpLST_sf <- house_districts %>% 
  select(REP_DIST) %>% 
  st_join(me_blkgrpLST_sf, ., largest = TRUE)

# Assign state senate district names to block groups
me_blkgrpLST_sf <- senate_districts %>% 
  select(SEN_DIST) %>% 
  st_join(me_blkgrpLST_sf, ., largest = TRUE)
...
muniDayNight_80th <- me_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  filter(percent_rank(meanAvgLST) >= 0.8 & POPSlabel != "No Pops of Concern") %>%
  group_by(TOWN) %>% 
  summarize(BGs80thDayNight = n())

houseDayNight_80th <- me_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  filter(percent_rank(meanAvgLST) >= 0.8 & POPSlabel != "No Pops of Concern") %>%
  group_by(REP_DIST) %>% 
  summarize(BGs80thDayNight = n())

senateDayNight_80th <- me_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  filter(percent_rank(meanAvgLST) >= 0.8 & POPSlabel != "No Pops of Concern") %>%
  group_by(SEN_DIST) %>% 
  summarize(BGs80thDayNight = n())
...
muniUHI24h_80th <- me_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  filter(percent_rank(UHI24avg) >= 0.8 & POPSlabel != "No Pops of Concern") %>% 
  group_by(TOWN) %>% 
  summarize(BGs80thUHI24h = n())

houseUHI24h_80th <- me_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  filter(percent_rank(UHI24avg) >= 0.8 & POPSlabel != "No Pops of Concern") %>% 
  group_by(REP_DIST) %>% 
  summarize(BGs80thUHI24h = n())

senateUHI24h_80th <- me_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  filter(percent_rank(UHI24avg) >= 0.8 & POPSlabel != "No Pops of Concern") %>% 
  group_by(SEN_DIST) %>% 
  summarize(BGs80thUHI24h = n())
...
muniNight_80th <- me_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  filter(percent_rank(meanNightLST) >= 0.8 & POPSlabel != "No Pops of Concern") %>% 
  group_by(TOWN) %>% 
  summarize(BGs80thNight = n())

houseNight_80th <- me_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  filter(percent_rank(meanNightLST) >= 0.8 & POPSlabel != "No Pops of Concern") %>% 
  group_by(REP_DIST) %>% 
  summarize(BGs80thNight = n())

senateNight_80th <- me_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  filter(percent_rank(meanNightLST) >= 0.8 & POPSlabel != "No Pops of Concern") %>% 
  group_by(SEN_DIST) %>% 
  summarize(BGs80thNight = n())
...
# create layer of me_blkgrpLST_sf for 80th percentile
me_blkgrps_sf_DayNight <- me_blkgrpLST_sf %>% 
...
me_blkgrps_sf_UHI24h <- me_blkgrpLST_sf %>% 
...
me_blkgrps_sf_Night <- me_blkgrpLST_sf %>% 
...
# clean up
rm(list = ls(pattern = paste("muni", "_80th", "houseN", "senateN", "houseL", "senateL", "houseU", "senateU", "houseD", "senateD", "me_blkgrpLST_sf$", sep = "|")))
```
...
Areas in and around I-95 as far north as Houlton experience the highest daytime temperatures. Higher nighttime temperatures are more widespread across the state and include much of the southern coastline. Limited English speaking households and people of color are more likely to experience the highest daytime and nighttime temperatures.

These interactive figures identify communities across Maine that are most overburdened and most vulnerable to three measures of ambient temperature: Day-Night average land surface temperatures (LST), the Urban Heat Island effect, and elevated nighttime LST.
...
pal <- colorQuantile(
  palette = "YlOrRd",
  domain = me_blkgrps_sf_DayNight$meanAvgLST,
  n = 5)
...
PopupHouse <- paste0("Maine state House District ", "<b>",house_districts$REP_DIST,"</b>", " has ", "<b>",house_districts$BGs80thDayNight,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing Day-Night average land surface temperatures at the 80th percentile - ", round(min(me_blkgrps_sf_DayNight$meanAvgLST),1),"° - or higher.", "<br/>",
...
PopupSenate <- paste0("Maine state Senate District ", "<b>",senate_districts$SEN_DIST,"</b>", " has ", "<b>",senate_districts$BGs80thDayNight,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing Day-Night average land surface temperatures at the 80th percentile - ", round(min(me_blkgrps_sf_DayNight$meanAvgLST),1),"° - or higher.", "<br/>",
...
Popup <- paste0(me_blkgrps_sf_DayNight$NAME, "<br/>",
                "<b>State House District:</b> ", me_blkgrps_sf_DayNight$REP_DIST, "<br/>",
                  "<b>State Senate District:</b> ", me_blkgrps_sf_DayNight$SEN_DIST, "<br/>",
                      "<b>Town:</b> ", me_blkgrps_sf_DayNight$TOWN, "<br/>",
                      "<b>Day-Night Avg Temp (°F):</b> ", round(me_blkgrps_sf_DayNight$meanAvgLST,1),"°", "<br/>",
         "<b>Day-Night Avg Temp percentile:</b> ", round(me_blkgrps_sf_DayNight$meanAvgLSTpctile*100,0), "<br/>",
                      "<b>Priority Populations: </b>", me_blkgrps_sf_DayNight$POPSlabel)

leaflet(width = "100%") %>% 
  addProviderTiles(providers$Stamen.TonerLite) %>% 
  addPolygons(data = me_towns_simple,
              weight = 0.7,
              opacity = 1,
              color = "gray",
              fillOpacity = 0,
  ...
  addPolygons(data = me_blkgrps_sf_DayNight, 
  ...
  addLegend(title = "Highest Day-Night<br>Avg LST (°F) and<br>High Priority Pops", 
            pal = pal,
            values = me_blkgrps_sf_DayNight$meanAvgLST,
            position = "bottomleft",
  ...
  setView(lng = -68.97, 45.37, zoom = 6) %>% 
  # addMiniMap() %>% 
...
pal <- colorQuantile(
  palette = "YlOrRd",
  domain = me_blkgrps_sf_UHI24h$UHI24avg,
  n = 5)
...
PopupHouse <- paste0("Maine state House District ", "<b>",house_districts$REP_DIST,"</b>", " has ", "<b>",house_districts$BGs80thDayNight,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing Day-Night average urban heat island effects at the 80th percentile - ", round(min(me_blkgrps_sf_UHI24h$UHI24avg),1),"° - or higher.", "<br/>",
...
PopupSenate <- paste0("Maine state Senate District ", "<b>",senate_districts$SEN_DIST,"</b>", " has ", "<b>",senate_districts$BGs80thDayNight,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing Day-Night average urban heat island effects at the 80th percentile - ", round(min(me_blkgrps_sf_UHI24h$UHI24avg),1),"° - or higher.", "<br/>",
...
Popup <- paste0(me_blkgrps_sf_UHI24h$NAME, "<br/>",
                "<b>State House District:</b> ", me_blkgrps_sf_UHI24h$REP_DIST, "<br/>",
                  "<b>State Senate District:</b> ", me_blkgrps_sf_UHI24h$SEN_DIST, "<br/>",
                      "<b>Town:</b> ", me_blkgrps_sf_UHI24h$TOWN, "<br/>",
                      "<b>Day-Night Avg UHI (°F):</b> ", round(me_blkgrps_sf_UHI24h$UHI24avg,1),"°", "<br/>",
         "<b>Day-Night Avg UHI percentile:</b> ", round(me_blkgrps_sf_UHI24h$UHI24avgpctile*100,0), "<br/>",
                      "<b>Priority Populations: </b>", me_blkgrps_sf_UHI24h$POPSlabel)
...
leaflet(width = "100%") %>% 
  ...
  addPolygons(data = senate_districts,
              ...
              highlight = highlightOptions(
                ...
                bringToFront = TRUE),
              group = "State Senate Districts") %>% 
  addPolygons(data = me_blkgrps_sf_UHI24h, 
              color = ~pal(UHI24avg),
              weight = 0.5,
              opacity = 0.7,
              # color = "white",
  ...
  addLegend(title = "Highest Day-Night<br>Urban Heat Island<br>effects (°F) and<br>High Priority Pops", 
            pal = pal,
            values = me_blkgrps_sf_UHI24h$UHI24avg,
            position = "bottomleft",
            labFormat = function(type, cuts, p) {
              n = length(cuts)
              p = paste0(round(p * 100), '%')
...
pal <- colorQuantile(
  palette = "YlOrRd",
  domain = me_blkgrps_sf_Night$meanNightLST,
  n = 5)
...
PopupHouse <- paste0("Maine state House District ", "<b>",house_districts$REP_DIST,"</b>", " has ", "<b>",house_districts$BGs80thNight,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing nighttime average land surface temperatures at the 80th percentile - ", round(min(me_blkgrps_sf_Night$meanNightLST),1),"° - or higher.", "<br/>",
...
PopupSenate <- paste0("Maine state Senate District ", "<b>",senate_districts$SEN_DIST,"</b>", " has ", "<b>",senate_districts$BGs80thNight,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing nighttime average land surface temperatures at the 80th percentile - ", round(min(me_blkgrps_sf_Night$meanNightLST),1),"° - or higher.", "<br/>",
...
Popup <- paste0(me_blkgrps_sf_Night$NAME, "<br/>",
                "<b>State House District:</b> ", me_blkgrps_sf_Night$REP_DIST, "<br/>",
                  "<b>State Senate District:</b> ", me_blkgrps_sf_Night$SEN_DIST, "<br/>",
                      "<b>Town:</b> ", me_blkgrps_sf_Night$TOWN, "<br/>",
                      "<b>Night Avg Temp (°F):</b> ", round(me_blkgrps_sf_Night$meanNightLST,1),"°", "<br/>",
         "<b>Night Avg Temp percentile:</b> ", round(me_blkgrps_sf_Night$meanNightLSTpctile*100,0), "<br/>",
                      "<b>Priority Populations: </b>", me_blkgrps_sf_Night$POPSlabel)
...
leaflet(width = "100%") %>% 
  ...
  addPolygons(data = senate_districts,
              ...
              highlight = highlightOptions(
                ...
                bringToFront = TRUE),
              group = "State Senate Districts") %>% 
  addPolygons(data = me_blkgrps_sf_Night, 
  ...
  addLegend(title = "Highest Nighttime<br>Avg LST (°F) and<br>High Priority Pops", 
            pal = pal,
            values = me_blkgrps_sf_Night$meanNightLST,
            position = "bottomleft",
            labFormat = function(type, cuts, p) {
              n = length(cuts)
              p = paste0(round(p * 100), '%')
...
[Moderate-resolution Imaging Spectroradiometer (MODIS)](https://modis.gsfc.nasa.gov/data/dataprod/mod11.php) satellite sensor. The three specific heat metrics highlighted here include: day-night average LST in the 80th percentile (i.e., `r round(min(me_blkgrps_sf_DayNight$meanAvgLST),1)`°F or warmer), day-night average urban heat island effects in the 80th percentile (i.e., `r round(min(me_blkgrps_sf_UHI24h$UHI24avg),1)`°F or warmer), and nighttime average LST in the 80th percentile (i.e., `r round(min(me_blkgrps_sf_Night$meanNightLST),1)`°F or warmer). These metrics are three of the six heat metrics used to calculate [cumulative Heat burdens](index.html).
...
### Day-Night Avg LST
```{r graphDayNight, fig.align="center", fig.cap="Population-weighted average exposures to day-night average temperatures for populations of concern in Maine."}
# Add status variable and recode demographic names
LST24hrPop %>% 
  mutate(
    Status = case_when(
           # Group == "Total Pop" ~ "State avg",
           LST24hrwMean == pull(LST24hrPop[LST24hrPop$Group=="Total Pop",2]) ~ "State avg",
...
### Urban Heat Island
```{r graphUHI, fig.align="center", fig.cap="Population-weighted average exposures to day-night average temperatures for populations of concern in Maine."}
# Add status variable and recode demographic names
UHI24hrPop %>% 
  mutate(
    Status = case_when(
...
### Nighttime Avg LST
```{r graphNight, fig.align="center", fig.cap="Population-weighted average exposures to nighttime average temperatures for populations of concern in Maine."}
# Add status variable and recode demographic names
LSTNightPop %>% 
  mutate(
    Status = case_when(
           # Group == "Total Pop" ~ "State avg",
           LSTNightwMean == pull(LSTNightPop[LSTNightPop$Group=="Total Pop",2]) ~ "State avg",
...
exLSTtown <- me_towns %>% as.data.frame() %>% filter(pctrank_meanAvgLST.mean > 95) %>% select(NAME) %>% slice_tail(n = 1) %>% pull()
exLST <- me_towns %>% as.data.frame() %>% filter(NAME == exLSTtown) %>% select(meanAvgLST.mean) %>% pull() %>% round(., 1)
exLSTpct <- me_towns %>% as.data.frame() %>% filter(NAME == exLSTtown) %>% select(pctrank_meanAvgLST.mean) %>% pull() %>% round(., 1)
