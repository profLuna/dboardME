...
NFHZAbyTown <- me_blkgrps_nfhza %>% 
  transmute(NewPop = NewPop, OldArea = as.numeric(st_area(.))) %>% 
  st_intersection(st_transform(me_towns, st_crs(me_blkgrps_nfhza))) %>% 
...
town_nfhzaPop_pct <- me_towns_sf %>% 
  as.data.frame() %>% 
  left_join(., town_pops, by = "GEOID") %>% 
  select(NAMELSAD, totalpopE) %>% 
  left_join(., NFHZAbyTown,by = "NAMELSAD") %>% 
  replace_na(list(nfhzaPop = 0)) %>% 
  mutate(totalpopE = if_else(totalpopE < nfhzaPop, nfhzaPop, totalpopE),
         PctPopNFHZA = if_else(totalpopE == 0, 0, nfhzaPop/totalpopE*100))


# repeat for hurricanes
# Calculate Pop and Pct pop at risk by jurisdiction
HEVACbyTown <- me_blkgrps_hevac %>% 
  transmute(NewPop = NewPop, OldArea = as.numeric(st_area(.))) %>% 
  st_intersection(st_transform(me_towns, st_crs(me_blkgrps_hevac))) %>% 
  mutate(NewArea = as.numeric(st_area(.)),
         Proportion = NewArea/OldArea,
...
house_names_pops <- get_acs(geography = "block group", survey = "acs5",
                     variables = c(totalpop = "B03002_001"), state = "ME", 
                     output = "wide", year = 2018, geometry = TRUE) %>% 
  select(GEOID, totalpopE) %>% 
  st_transform(.,st_crs(me_blkgrps_nfhza)) %>% 
  select(totalpopE) %>% 
  mutate(OldArea = as.numeric(st_area(.))) %>% 
    st_intersection(
      st_make_valid(
        st_transform(house_districts,st_crs(me_blkgrps_nfhza))
        )
      ) %>% 
  mutate(NewArea = as.numeric(st_area(.)),
         Proportion = NewArea/OldArea,
...
house_floodPop_pct <- me_blkgrps_hevac %>% 
  transmute(NewPop = NewPop, OldArea = as.numeric(st_area(.))) %>% 
  st_intersection(
    st_make_valid(
      st_transform(house_districts, st_crs(me_blkgrps_hevac))
      )
    ) %>% 
  mutate(NewArea = as.numeric(st_area(.)),
         Proportion = NewArea/OldArea,
...
senate_names_pops <- get_acs(geography = "block group", survey = "acs5",
  ...
  mutate(OldArea = as.numeric(st_area(.))) %>% 
    st_intersection(
      st_make_valid(
        st_transform(senate_districts,st_crs(me_blkgrps_nfhza))
        )
      ) %>% 
  mutate(NewArea = as.numeric(st_area(.)),
         Proportion = NewArea/OldArea,
...
# identify FEMA risk pops by district and calculate pop pct
NFHZAbySenate <- me_blkgrps_nfhza %>% 
  transmute(NewPop = NewPop, OldArea = as.numeric(st_area(.))) %>% 
  st_intersection(
    st_make_valid(
      st_transform(senate_districts, st_crs(me_blkgrps_nfhza))
...
# identify HEVAC risk pops by district and calculate pop pct, and bring together with FEMA
senate_floodPop_pct <- me_blkgrps_hevac %>% 
  transmute(NewPop = NewPop, OldArea = as.numeric(st_area(.))) %>% 
  st_intersection(
    st_make_valid(
      st_transform(senate_districts, st_crs(me_blkgrps_hevac))
      )
    ) %>% 
  mutate(NewArea = as.numeric(st_area(.)),
         Proportion = NewArea/OldArea,
...
muniNFHZA <- me_blkgrps_sf %>%
  as.data.frame() %>%
  filter(nfhzaRisk == "F" & POPSlabel != "No Pops of Concern") %>%
  group_by(TOWN) %>%
  summarize(BGsNFHZA = n())

houseNFHZA <- me_blkgrps_sf %>%
  as.data.frame() %>%
  filter(nfhzaRisk == "F" & POPSlabel != "No Pops of Concern") %>%
  group_by(REP_DIST) %>%
  summarize(BGsNFHZA = n())

senateNFHZA <- me_blkgrps_sf %>%
  as.data.frame() %>%
  filter(nfhzaRisk == "F" & POPSlabel != "No Pops of Concern") %>%
  group_by(SEN_DIST) %>%
  summarize(BGsNFHZA = n())
...
muniHEVAC <- me_blkgrps_sf %>%
  as.data.frame() %>%
  filter(hevacRisk == "H" & POPSlabel != "No Pops of Concern") %>%
  group_by(TOWN) %>%
  summarize(BGsHEVAC = n())

houseHEVAC <- me_blkgrps_sf %>%
  as.data.frame() %>%
  filter(hevacRisk == "H" & POPSlabel != "No Pops of Concern") %>%
  group_by(REP_DIST) %>%
  summarize(BGsHEVAC = n())

senateHEVAC <- me_blkgrps_sf %>%
  as.data.frame() %>%
  filter(hevacRisk == "H" & POPSlabel != "No Pops of Concern") %>%
  group_by(SEN_DIST) %>%
  summarize(BGsHEVAC = n())
...
# create layer of me_blkgrps_sf for flood and hurricane
me_blkgrps_sf_NFHZA <- me_blkgrps_sf %>% 
  filter(nfhzaRisk == "F" & POPSlabel != "No Pops of Concern") %>% 
  select(NAME, REP_DIST, SEN_DIST, TOWN, POPSlabel, floodtypes) %>% 
  ms_simplify(., keep = 0.1, keep_shapes = TRUE)

me_blkgrps_sf_HEVAC <- me_blkgrps_sf %>% 
  filter(hevacRisk == "H" & POPSlabel != "No Pops of Concern") %>% 
...
At least 33,000 people in Maine live in a FEMA flood risk zone and over 59,000 live in a hurricane inundation zone. Households without a car, those over 64, low income persons, and the disabled are more likely to live in flood zones compared to the general population or other priority populations. Limited English speaking households, households without a car, and people of color are more likely to live in a hurricane inundation zone compared to the general population or other priority populations.
...
PopupHouse <- paste0("Maine state House District ", "<b>",house_districts$REP_DIST,"</b>", " has ", "<b>",house_districts$BGsNFHZA,"</b>", " <b>Block Groups</b> with high percentages of priority populations living within FEMA flood zones.")

PopupSenate <- paste0("Maine state Senate District ", "<b>",senate_districts$SEN_DIST,"</b>", " has ", "<b>",senate_districts$BGsNFHZA,"</b>", " <b>Block Groups</b> with high percentages of priority populations living within FEMA flood zones.")

Popup <- paste0(me_blkgrps_sf_NFHZA$NAME, "<br/>",
                "<b>State House District:</b> ", me_blkgrps_sf_NFHZA$REP_DIST, "<br/>",
                "<b>State Senate District:</b> ", me_blkgrps_sf_NFHZA$SEN_DIST, "<br/>",
                "<b>Town:</b> ", me_blkgrps_sf_NFHZA$TOWN, "<br/>",
                "<b>Priority Populations: </b>", me_blkgrps_sf_NFHZA$POPSlabel, "<br/>",
                "<b>FEMA flood risks: </b> ", me_blkgrps_sf_NFHZA$floodtypes)
...
leaflet(width = "100%") %>% 
  ...
  addPolygons(data = me_blkgrps_sf_NFHZA, 
              color = "red",
              weight = 0.5,
              opacity = 0.7,
              # color = "white",
...
PopupHouse <- paste0("Maine state House District ", "<b>",house_districts$REP_DIST,"</b>", " has ", "<b>",house_districts$BGsHEVAC,"</b>", " <b>Block Groups</b> with high percentages of priority populations living within hurricane evacuation zones.")

PopupSenate <- paste0("Maine state Senate District ", "<b>",senate_districts$SEN_DIST,"</b>", " has ", "<b>",senate_districts$BGsHEVAC,"</b>", " <b>Block Groups</b> with high percentages of priority populations living within hurricane evacuation zones.")

Popup <- paste0(me_blkgrps_sf_HEVAC$NAME, "<br/>",
                "<b>State House District:</b> ", me_blkgrps_sf_HEVAC$REP_DIST, "<br/>",
                "<b>State Senate District:</b> ", me_blkgrps_sf_HEVAC$SEN_DIST, "<br/>",
                "<b>Town:</b> ", me_blkgrps_sf_HEVAC$TOWN, "<br/>",
                "<b>Priority Populations: </b>", me_blkgrps_sf_HEVAC$POPSlabel)
...
leaflet(width = "100%") %>% 
  ...
  addPolygons(data = senate_districts,
              ...
              highlight = highlightOptions(
                ...
                bringToFront = TRUE),
              group = "State Senate Districts") %>% 
  addPolygons(data = me_blkgrps_sf_HEVAC, 
              color = "red",
              weight = 0.5,
              opacity = 0.7,
              # color = "white",
...
me_FloodPops_df %>% 
  mutate(
    Status = case_when(
           ...
           PctFlood < me_FloodPops_df[me_FloodPops_df$Group=="Total Pop",4] ~ "Below state avg",
           PctFlood > me_FloodPops_df[me_FloodPops_df$Group=="Total Pop",4] ~ "Above state avg"),
  ...
  hc_title(text = "Maine Populations Living within Flood Zones", useHTML = TRUE)
```
...
```{r graphHEVAC, fig.align="center", fig.cap="Maine Populations Living within Hurricane Inundation Zones."}
# Add status variable and recode demographic names
me_HevacPops_df %>% 
  mutate(
    Status = case_when(
           Group == "Total Pop" ~ "State avg",
           PctHevac < me_HevacPops_df[me_HevacPops_df$Group=="Total Pop",4] ~ "Below state avg",
           PctHevac > me_HevacPops_df[me_HevacPops_df$Group=="Total Pop",4] ~ "Above state avg"),
    Group = recode(Group, "Minority" = "People of Color",
                        "No HS Dip" = "No HS Diploma"),
  ...
  hc_xAxis(title = NULL, type = "category", labels = list(step = 1)) %>% 
  hc_tooltip(pointFormat = "{series.name}: {point.y}%") %>% 
  hc_title(text = "Maine Populations Living within Hurricane Evacuation Zones", useHTML = TRUE)
```
...
* The analysis of hurricane inundation risk presented here is based on Hurricane Evacuation Maps produced by the [U.S. Army Corps of Engineers for New England](https://www.nae.usace.army.mil/Missions/Projects-Topics/Maine-Hurricane-Studies/). The maps show Hurricane Inundation Zones that are recommended to be evacuated during potential worst-case hurricane storm surge inundation. These inundation zones are based on storm surge modeling by the National Weather Service’s National Hurricane Center using the hydrodynamic [Sea, Lake, and Overland Surges from Hurricanes (SLOSH) model](https://www.nhc.noaa.gov/nationalsurge/) to simulate storm surge from tropical cyclones. 
