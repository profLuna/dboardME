...
# Read in no transit access. Whole census units with no transit access. Includes me_busBuff400m_sf with counts of pop with access to bus. 
...
me_blkgrps_sf <- readRDS(file = "../DATA/me_blkgrps_sf_CUM.Rds")
...
me_blkgrps_sf <- house_districts %>% 
  select(REP_DIST) %>% 
  st_join(me_blkgrps_sf, ., largest = TRUE)

# Assign state senate district names to block groups
me_blkgrps_sf <- senate_districts %>% 
  select(SEN_DIST) %>% 
  st_join(me_blkgrps_sf, ., largest = TRUE)
...
muniNoBus <- me_blkgrps_sf %>% 
  as.data.frame() %>% 
  filter(NoBusAccess == "Y" & POPSlabel != "No Pops of Concern") %>% 
  group_by(TOWN) %>% 
  summarize(BGsNoBus = n())

houseNoBus <- me_blkgrps_sf %>% 
  as.data.frame() %>% 
  filter(NoBusAccess == "Y" & POPSlabel != "No Pops of Concern") %>% 
  group_by(REP_DIST) %>% 
  summarize(BGsNoBus = n())

senateNoBus <- me_blkgrps_sf %>% 
  as.data.frame() %>% 
  filter(NoBusAccess == "Y" & POPSlabel != "No Pops of Concern") %>% 
  group_by(SEN_DIST) %>% 
  summarize(BGsNoBus = n())
...
muniTcost_80th <- me_blkgrps_sf %>% 
  as.data.frame() %>% 
  filter(Tcostpctile >= 80 & POPSlabel != "No Pops of Concern") %>% 
  group_by(TOWN) %>% 
  summarize(BGs80thTcost = n())

houseTcost_80th <- me_blkgrps_sf %>% 
  as.data.frame() %>% 
  filter(Tcostpctile >= 80 & POPSlabel != "No Pops of Concern") %>% 
  group_by(REP_DIST) %>% 
  summarize(BGs80thTcost = n())

senateTcost_80th <- me_blkgrps_sf %>% 
  as.data.frame() %>% 
  filter(Tcostpctile >= 80 & POPSlabel != "No Pops of Concern") %>% 
  group_by(SEN_DIST) %>% 
  summarize(BGs80thTcost = n())
...
# create layer of me_blkgrp_sf for 80th percentile and thresholds
me_blkgrps_sf_NoBus <- me_blkgrps_sf %>% 
  filter(NoBusAccess == "Y" & POPSlabel != "No Pops of Concern") %>% 
  select(NAME, REP_DIST, SEN_DIST, TOWN, POPSlabel) %>% 
  ms_simplify(., keep = 0.1, keep_shapes = TRUE)

me_blkgrps_sf_Walk <- me_blkgrps_sf %>% 
  filter(NatWalkInd < 5.76 & POPSlabel != "No Pops of Concern") %>% 
  select(NAME, REP_DIST, SEN_DIST, TOWN, POPSlabel, NatWalkInd, Walkpctile, 
         WalkCat) %>% 
  ms_simplify(., keep = 0.1, keep_shapes = TRUE)

me_blkgrps_sf_Tcost <- me_blkgrps_sf %>% 
  filter(Tcostpctile >= 80 & POPSlabel != "No Pops of Concern") %>% 
  select(NAME, REP_DIST, SEN_DIST, TOWN, POPSlabel, hh7_t, Tcostpctile) %>% 
  ms_simplify(., keep = 0.1, keep_shapes = TRUE)

# extract values for inline insertion
Tcost80th <- round(quantile(me_blkgrps_sf$hh7_t, 0.8, na.rm = T),1)

# clean up "me_blkgrps_sf$",
rm(list = ls(pattern = paste("muni", "_80th", "headway80th", "_tract", "houseNoBus", "Transport", "houseWalk", "senateNoBus", "senateWalk", "lai_me_blkgrp", "lai_ToverH", "_noTransit", "me_blkgrps_sf_noBus", sep = "|"), ))
```
---
# Transport
...
* Most of Maine’s bus services are concentrated in a handful of cities, mostly in the southeast corner of the state. 
* Approximately 73% of the state’s population does not live within a reasonable distance of a bus route. 
* Transportation cost burdens for moderate income households are the highest in New England. 
* The average transportation cost burden is 31% of household income for moderate income households, which is almost twice the US average. 
* There are 202 census tracts where the transportation cost burdens exceeds housing cost burdens. This is 56% of all census tracts in the state. These were mostly found in the northern and western parts of Maine. 
* The lowest transportation cost burden was found in the southeast corner of the state from Kittery to Brunswick.

These interactive figures identify communities across Maine that are most underserved and most sensitive to the quality of four transportation metrics: access to public transit, transit headways, neighborhood walkability, and transportation cost burden.  
...
#   palette = "YlOrRd",
#   domain = me_blkgrps_sf_PM25$PM25_19,
#   n = 5)
...
PopupHouse <- paste0("Maine state House District ", "<b>",house_districts$REP_DIST,"</b>", " has ", "<b>",house_districts$BGsNoBus,"</b>", " <b>Block Groups</b> with high percentages of priority populations and no reasonable access to a bus.")

PopupSenate <- paste0("Maine state Senate District ", "<b>",senate_districts$SEN_DIST,"</b>", " has ", "<b>",senate_districts$BGsNoBus,"</b>", " <b>Block Groups</b> with high percentages of priority populations and no reasonable access to a bus.")

Popup <- paste0(me_blkgrps_sf_NoBus$NAME, "<br/>",
                "<b>State House District:</b> ", me_blkgrps_sf_NoBus$REP_DIST, "<br/>",
                  "<b>State Senate District:</b> ", me_blkgrps_sf_NoBus$SEN_DIST, "<br/>",
                      "<b>Town:</b> ", me_blkgrps_sf_NoBus$TOWN, "<br/>",
                      "<b>Priority Populations: </b>", me_blkgrps_sf_NoBus$POPSlabel)
...
leaflet(width = "100%") %>% 
  ...
  addPolygons(data = me_blkgrps_sf_NoBus, 
              color = "red",
              weight = 0.5,
              opacity = 0.7,
              # color = "white",
...
palWalk <- colorQuantile(
  palette = "YlOrRd",
  domain = me_blkgrps_sf_Walk$NatWalkInd,
  n = 5,
  reverse = TRUE)
...
PopupHouse <- paste0("Maine state House District ",
...
PopupSenate <- paste0("Maine state Senate District ", "<b>",senate_districts$SEN_DIST,"</b>", " has ", "<b>",senate_districts$BGsLWalk,"</b>", " <b>Block Groups</b> with high percentages of priority populations in least walkable neighborhoods.", "<br/>",
...
Popup <- paste0(me_blkgrps_sf_Walk$NAME, "<br/>",
                "<b>State House District:</b> ", me_blkgrps_sf_Walk$REP_DIST, "<br/>",
                  "<b>State Senate District:</b> ", me_blkgrps_sf_Walk$SEN_DIST, "<br/>",
                      "<b>Town:</b> ", me_blkgrps_sf_Walk$TOWN, "<br/>",
                      "<b>Walkability Score:</b> ", paste0(round(me_blkgrps_sf_Walk$NatWalkInd,1), " (", me_blkgrps_sf_Walk$WalkCat,")"), "<br/>",
         "<b>Walkability percentile:</b> ", round(me_blkgrps_sf_Walk$Walkpctile,0), "<br/>",
                      "<b>Priority Populations: </b>", me_blkgrps_sf_Walk$POPSlabel)
...
leaflet(width = "100%") %>% 
  ...
  addPolygons(data = senate_districts,
              ...
              highlight = highlightOptions(
                ...
                bringToFront = TRUE),
              group = "State Senate Districts") %>% 
  addPolygons(data = me_blkgrps_sf_Walk, 
  ...
  addLegend(title = "Least Walkable Scores<br>for Priority Populations", 
            pal = palWalk,
            values = me_blkgrps_sf_Walk$NatWalkInd,
            position = "bottomleft",
...
palTcost <- colorQuantile(
  palette = "YlOrRd",
  domain = me_blkgrps_sf_Tcost$hh7_t,
  n = 5)
...
PopupHouse <- paste0("Maine state House District ", "<b>",house_districts$REP_DIST,"</b>", " has ", "<b>",house_districts$BGs80thTcost,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing transportation cost burdens at the 80th percentile - ", Tcost80th,"% of household income - or higher.", "<br/>",
...
PopupSenate <- paste0("Maine state Senate District ", "<b>",senate_districts$SEN_DIST,"</b>", " has ", "<b>",senate_districts$BGs80thTcost,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing transportation cost burdens at the 80th percentile - ", Tcost80th,"% of household income - or higher.", "<br/>",
...
Popup <- paste0(me_blkgrps_sf_Tcost$NAME, "<br/>",
                "<b>State House District:</b> ", me_blkgrps_sf_Tcost$REP_DIST, "<br/>",
                  "<b>State Senate District:</b> ", me_blkgrps_sf_Tcost$SEN_DIST, "<br/>",
                      "<b>Town:</b> ", me_blkgrps_sf_Tcost$TOWN, "<br/>",
                      "<b>Avg Transport Cost Burden:</b> ", round(me_blkgrps_sf_Tcost$hh7_t,1), "% of household income", "<br/>",
                      "<b>Avg Transport Cost Burden percentile: </b>", round(me_blkgrps_sf_Tcost$Tcostpctile,1), "<br/>",
                      "<b>Priority Populations: </b>", me_blkgrps_sf_Tcost$POPSlabel)
...
leaflet(width = "100%") %>% 
  ...
  addPolygons(data = senate_districts,
              ...
              highlight = highlightOptions(
                ...
                bringToFront = TRUE),
              group = "State Senate Districts") %>% 
  addPolygons(data = me_blkgrps_sf_Tcost, 
  ...
  addLegend(title = "Highest Transport Cost Burdens<br>(% Household Income)<br>for Priority Populations", 
            pal = palTcost,
            values = me_blkgrps_sf_Tcost$hh7_t,
            position = "bottomleft",
            labFormat = function(type, cuts, p) {
              n = length(cuts)
              p = paste0(round(p * 100), '%')
...
```{r graphBusAccess, fig.align="center", fig.cap="Percentage of population groups within 1/4 mile (400 meters) of a bus stop across Maine."}
me_transitAccessPops_df %>% 
  arrange(PctBus) %>% 
  mutate(PctBus = round(PctBus,1), 
         Status = case_when(
           Group == "Total Pop" ~ "State avg",
           PctBus < me_transitAccessPops_df[1,4] ~ "Below state avg",
           PctBus > me_transitAccessPops_df[1,4] ~ "Above state avg"),
  ...
  hc_title(text = "Maine Populations within Walking Distance (400m) of Bus Stops", 
           useHTML = TRUE)
```
...
```{r graphWalk, fig.align="center", fig.cap="Population-weighted average walkability index scores for groups across Maine."}
popAvgWalk <- me_wAvgWalkability_df %>% 
...
```{r graphTcost, fig.align="center", fig.cap="Population-weighted average transportation cost burden as a percentage of household income for groups across Maine."}
# extract total pop value
popAvgTcost <- lai_me_df %>% 
  filter(Group == "Total Pop") %>% 
  select(wTBurden) %>% 
  pull()
...
For example, adults over the age of 64 (i.e., 'Over 64') in the state reside in Census block groups that experience population-weighted transportation cost burdens of over `r round(lai_me_df %>% filter(Group == "Over 64") %>% select(wTBurden) %>% pull(),1)`% of annual household income. This is almost twice the national average of 16%. 
...
busAccesbyTown <- me_busBuff400m_sf %>% 
  transmute(NewPop = NewPop, OldArea = as.numeric(st_area(.))) %>% 
  st_intersection(st_transform(me_towns, st_crs(me_busBuff400m_sf))) %>% 
  mutate(NewArea = as.numeric(st_area(.)),
...
me_towns %>% 
  ...
  mutate(`Pct Bus Access` = 
           if_else(`Pct Bus Access` > 1, 1, `Pct Bus Access`)) %>% 
  arrange(`City/Town`) %>% 
...
busAccesbyHouse <- me_busBuff400m_sf %>% 
  transmute(NewPop = NewPop, OldArea = as.numeric(st_area(.))) %>% 
  st_intersection(
    st_make_valid(
    st_transform(house_districts,st_crs(me_busBuff400m_sf))
    )
    ) %>% 
  mutate(NewArea = as.numeric(st_area(.)),
         Proportion = NewArea/OldArea,
...
house_names_pops <- me_blkgrps_sf %>% 
  st_transform(.,st_crs(me_busBuff400m_sf)) %>% 
  select(totalpopE) %>% 
  mutate(OldArea = as.numeric(st_area(.))) %>% 
    st_intersection(
      st_make_valid(
...
# calculate bus access pops per district
busAccesbySenate <- me_busBuff400m_sf %>% 
  transmute(NewPop = NewPop, OldArea = as.numeric(st_area(.))) %>% 
  st_intersection(
    st_make_valid(
    st_transform(senate_districts,st_crs(me_busBuff400m_sf))
    )
    ) %>% 
  mutate(NewArea = as.numeric(st_area(.)),
         Proportion = NewArea/OldArea,
