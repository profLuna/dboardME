...
meta() %>%
  meta_description(
    "Environmental justice analysis of transportation-related burdens and benefits in Maine") %>% 
  meta_viewport() %>% 
  meta_social(
    title = "Investing for Equity in Maine",
    url = "http://dgl.salemstate.edu/luna/InvestingForEquity/ME/",
    og_type = "website",
...
house_districts <- st_read("../DATA/shapefiles/Maine_House_Districts_2013_GeoLibrary-shp", "Maine_House_Districts_2013_GeoLibrary") %>%
  ...
  st_transform(., crs = 4326) %>%
  st_make_valid() %>% 
  left_join(., read_csv("../tables/ME_burdensCnt_house.csv"), 
...
senate_districts <- st_read("../DATA/shapefiles/Maine_State_Senate_Districts_2013_GeoLibrary-shp", "Maine_State_Senate_Districts_2013_GeoLibrary") %>%
  ...
  st_transform(., crs = 4326) %>%
  st_make_valid() %>% 
  left_join(., read_csv("../tables/ME_burdensCnt_senate.csv"), 
...
# reduce number of variables in me_blkgrps_sf for faster loading AND reduce number of vertices
me_blkgrps_sf <- me_blkgrps_sf %>%
...
Our analysis shows that areas with high percentages of priority populations and the highest rankings of multiple cumulative environmental burdens fall in the southern third of the state, particularly around the largest urban areas and along the I-95 corridor. Limited English speaking households and people of color most frequently experience the greatest number and types of burdens, especially for air quality, heat, and flood or evacuation risks. Priority populations, especially adults with lower levels of education, disabilities, and in lower income households, in rural and suburban areas, lack transportation options and face high costs for transportation.
...
pal <- colorFactor(
  palette = "YlOrRd",
  domain = me_blkgrps_sf$BurdenCountLabel)

# alternatively, create a colors vector to pair with arbitrary labels
colors <- brewer.pal(n = 4, "YlOrRd")

PopupHouse <- paste0("Maine state House District ", "<b>",house_districts$REP_DIST,"</b>", " has ", "<b>",house_districts$`3 Burdens`,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing <b>3 categories</b> of highest cumulative burdens, and ", "<b>",house_districts$`4 Burdens`,"</b>", " <b>Block Groups</b> experiencing <b>4 categories</b> of highest cumulative burdens.")

PopupSenate <- paste0("Maine state Senate District ", "<b>",senate_districts$SEN_DIST,"</b>", " has ", "<b>",senate_districts$`3 Burdens`,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing <b>3 categories</b> of highest cumulative burdens, and ", "<b>",senate_districts$`4 Burdens`,"</b>", " <b>Block Groups</b> experiencing <b>4 categories</b> of highest cumulative burdens.")

Popup <- paste0(me_blkgrps_sf$NAME, "<br/>",
                "<b>State House District:</b> ", me_blkgrps_sf$REP_DIST, "<br/>",
                  "<b>State Senate District:</b> ", me_blkgrps_sf$SEN_DIST, "<br/>",
                      "<b>Town:</b> ", me_blkgrps_sf$TOWN, "<br/>",
                      "<b>Number of Burdens:</b> ", me_blkgrps_sf$BurdenCount, "<br/>",
                      "<b>Burden Categories: </b>", me_blkgrps_sf$BURDENSlabel,"<br/>",
                      "<b>Priority Populations: </b>", me_blkgrps_sf$POPSlabel)

leaflet(width = "100%") %>% 
  addProviderTiles(providers$Stamen.TonerLite) %>% 
  addPolygons(data = me_towns,
              weight = 0.7,
              opacity = 1,
              color = "gray",
              fillOpacity = 0,
  ...
  addPolygons(data = me_blkgrps_sf,
  ...
  addLegend(title = "Cumulative Burdens", 
            ...
            values = me_blkgrps_sf$BurdenCountLabel,
...
me_cum_burdens <- read_csv("../tables/ME_cum_burden.csv") %>% 
  as.data.frame() %>% 
...
LEH1 <- me_cum_burdens %>% filter(Group == "Limited English HH") %>% select(`1 Burden`) %>% pull()
LEH2 <- me_cum_burdens %>% filter(Group == "Limited English HH") %>% select(`2 Burdens`) %>% pull()
LEH4 <- me_cum_burdens %>% filter(Group == "Limited English HH") %>% rowwise %>% summarize(Total = sum(c_across(2:5))) %>% pull()
THH <- me_cum_burdens %>% filter(Group == "Total HH") %>% rowwise %>% summarize(Total = sum(c_across(2:5))) %>% pull()
...
categories_column <- names(me_cum_burdens[,1])
measure_columns <- names(me_cum_burdens[,2:5])

stacked_bar_chart(data = me_cum_burdens,
...
read_csv("../tables/ME_burdens_town.csv") %>% 
...
read_csv("../tables/ME_burdens_house.csv") %>% 
  mutate(across(starts_with("Pct"), ~./100)) %>% # let DT do formatting
  datatable(., rownames = FALSE, options = list(pageLength = 10), 
            container = sketch2) %>% 
  formatRound(., columns = c(2,4,6), digits = 0, mark = ",") %>% 
...
read_csv("../tables/ME_burdens_senate.csv") %>% 
  mutate(across(starts_with("Pct"), ~./100)) %>% # let DT do formatting
  datatable(., rownames = FALSE, options = list(pageLength = 10), 
            container = sketch3) %>% 
  formatRound(., columns = c(2,4,6), digits = 0, mark = ",") %>% 
